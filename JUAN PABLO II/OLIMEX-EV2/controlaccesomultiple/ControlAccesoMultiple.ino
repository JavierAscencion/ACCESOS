...
uint8_t contadorIntentaReconexion = 0;
uint8_t contadorEstadoSocketIO = 0;

const char *ssid = "RED ACCESA";
const char *pass = "037E32E7";
//const char *pass = "123456789A";
...
void IRAM_ATTR timerInterrupt()
{
  ...
  if (serverWaitReset)
  {
    ...
    // Realizar reboot
    if (++tempServer >= 4 * 15)
    { // Cada minuto restablece el sistema
...
void setup()
{
  ...
  //////////////////////////////core para monitorear conexion con wifi/////////////////////////////

  xTaskCreatePinnedToCore(
      BuscarWiFi,        // Function to implement the task
      "TareaBuscarWiFi", // Name of the task
...
void loop()
{
  ...
  { //==1
    if (serialBuffer.isNewMessage)
    {
      ...
      {
        if (!strncmp("-NIP:", &message[4], 5))
        { // MUXA-NIP:2585
          if (strlen(&message[9]) == 4)
          { // Validamos el nip recepcionado
            String json;
        ...
        else if (!strncmp("-IDX:", &message[4], 5))
        { // Lectura de tarjeta/tag
          if (requestAvailable)
          {
...
void eventos_tcp()
{
  ...
  { // Posicionar el archivo en la posicion
          ...
          { // Linea detectada
            ...
            if (!strncmp(buff, &message[9], 8))
            {
              ...
              { // Vigencia
                ...
                Serial.println();
                if (buff[11] != modeAccess || !isPassback)
                { // Passback
                  // Validamos el acceso
                  if (modeAccess == 'I')
                  {
...
void serialEventData()
{
  ...
  while (Serial3.available())
  {
    ...
    // Proteccion overflow
    if (serialBuffer.tam > (sizeof(serialBuffer.message) - 1))
    { // Overflow
      serialBuffer.tam = 0;
